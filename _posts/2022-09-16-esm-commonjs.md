...
CommonJS import는 런타임에서 동적으로 실행된다. `require()` 함수는 단순하게 코드가 실행될 때 동작하며 결과적으로 코드의 어느 곳이든 부를 수 있다.

ES 모듈의 import는 정적이고 구문 분석 시간에 실행된다. 이는 import가 **호이스팅**된다고 볼 수 있다. import가 암시적으로 파일의 맨 위로 옮겨지기 때문에 코드의 중간에서 import를 할 수가 없는 것이다. 이는 에러를 사전에 확인할 수 있고 개발자 도구를 사용해 더 유효한 코드를 작성할 수 있는 장점이 있다.
